...
export class MultimodalLiveClient extends EventEmitter {
  ...
  constructor({ url, apiKey }) {
    ...
    this.url =
      url ||
      `wss://wsgemini.vvangxuanan.top/ws/google.ai.generativelanguage.v1alpha.GenerativeService.BidiGenerateContent?key=${apiKey}`
    this.ws = null
    this.config = null
  ...
  connect(config) {
    ...
    return new Promise((resolve, reject) => {
      const onError = (ev) => {
        ...
        throw new ApplicationError(message, ErrorCodes.WEBSOCKET_CONNECTION_FAILED, { originalError: ev })
      }
